...
(defun lisatiy/cleanup-recentf ()
  (progn
    (and (fboundp 'recentf-cleanup)
         (recentf-cleanup))))

(add-hook 'kill-emacs-hook #'lisatiy/cleanup-recentf)
...
(defun lisatiy/stop-using-minibuffer ()
  "kill the minibuffer"
  (when (and (>= (recursion-depth) 1) (active-minibuffer-window))
    (abort-recursive-edit)))

(add-hook 'mouse-leave-buffer-hook 'lisatiy/stop-using-minibuffer)
...
(defun lisatiy-ag-edit (function)
...
;; (defun lisatiy-after-ag-edit (function)
;;   (ivy-occur-grep-mode))
