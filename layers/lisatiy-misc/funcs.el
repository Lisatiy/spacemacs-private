;;; funcs.el --- lisatiy Layer packages File for Spacemacs
;;
;; Copyright (c) 2015-2016 lisatiy 
;;
...
(defun lisatiy/clearn-highlight ()
    (interactive)
...
(defun lisatiy/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun lisatiy/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun lisatiy/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun lisatiy/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun lisatiy/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "lisatiy")))

(defun lisatiy/save-my-layout ()
  (interactive)
  (persp-save-state-to-file (concat persp-save-dir "lisatiy")))
...
(defun lisatiy/indent-region(numSpaces)
  (progn
...
(defun lisatiy/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (lisatiy/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun lisatiy/untab-region (N)
  (interactive "p")
  (lisatiy/indent-region -4))

(defun lisatiy/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'lisatiy/tab-region)
  (local-set-key (kbd "<S-tab>") 'lisatiy/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'lisatiy/hack-tab-key)
...
(defun lisatiy/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(lisatiy//hotspots-sources))))

(defun lisatiy//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (lisatiy/run-current-file)))
...
(defun lisatiy/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun lisatiy/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (lisatiy/git-project-root)
      (counsel-git)
    (if (projectile-project-p)
        (projectile-find-file)
      (counsel-file-jump))))

(defun lisatiy/pomodoro-notification ()
  ...
  (if (spacemacs/system-is-mswindows)
    ...
    (progn (add-hook 'org-pomodoro-finished-hook '(lambda () (lisatiy/growl-notification "Pomodoro Finished" "?? Have a break!" t)))
           (add-hook 'org-pomodoro-short-break-finished-hook '(lambda () (lisatiy/growl-notification "Short Break" "?? Ready to Go?" t)))
             (add-hook 'org-pomodoro-long-break-finished-hook '(lambda () (lisatiy/growl-notification "Long Break" " ?? Ready to Go?" t))))))
...
(defun lisatiy/growl-notification (title message &optional sticky)
...
(defun lisatiy/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (lisatiy/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun lisatiy/goto-match-paren (arg)
...
(defun lisatiy/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun lisatiy/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun lisatiy/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (lisatiy/retrieve-chrome-current-tab-url)))

(defun lisatiy/list-all-tabs ()
  (interactive)
  (ivy-read
...
(defun lisatiy/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun lisatiy/single-lines-only ()
...
(defun lisatiy/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun lisatiy/ash-term-hooks ()
...
(defun lisatiy/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun lisatiy/evil-quick-replace (beg end )
  (interactive "r")
...
(defun lisatiy/git-project-root ()
...
(defun lisatiy/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (lisatiy/git-project-root)
                default-directory))
...
(defun lisatiy/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (lisatiy/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun lisatiy/magit-visit-pull-request ()
...
(defun lisatiy/markdown-to-html ()
  (interactive)
...
(defun lisatiy/github-browse-commit ()
...
(defun lisatiy/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun lisatiy/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun lisatiy/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
