...
(defun lisatiy/directory-parent (directory)
...
;; Screenshot
(defun lisatiy//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun lisatiy/capture-screenshot (basename)
...
(defun lisatiy/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun lisatiy/skip-non-stuck-projects ()
...
(defun lisatiy/org-insert-src-block (src-code-type)
...
(defun lisatiy/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun lisatiy/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
    ...
    (while (setq file (pop files))
      ...
      (with-current-buffer org-agenda-buffer
        (dolist (current-tag include-tags)
          (org-clock-sum tstart tend 'lisatiy/filter-by-tags)
